...
# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 01:09:58 2019
...
def intersection_align_gensim(m1,m2, words=None):
	...
	for m in [m1,m2]:
		...
		m.wv.vectors_norm = m.syn0 = new_arr
...
def smart_procrustes_align_gensim(base_embed, other_embed, words=None):
	...
	other_embed.wv.vectors_norm = other_embed.syn0 = (other_embed.wv.vectors_norm).dot(ortho)
	return other_embed
...
def align_and_produce_new_model(earth_model, moon_sentences):
    ...
    moon_model = Word2Vec(sentences, size = 100, min_count = 0, iter = 200, 
...
def produce_model_stats(model):
    ...
    while True:
        try:
            gender.append(model.similarity('equality','gender'))
            break
        except KeyError:
            gender.append('NA')
            break
    while True:
        try:
            intl.append(model.similarity('equality','treaty'))
            break
        except KeyError:
            intl.append('NA')
            break
    while True:
        try:
            german.append(model.similarity('equality','german'))
            break
        except KeyError:
            german.append('NA')
            break
    while True:
        try:
            race.append(model.similarity('equality','race'))
            break
        except KeyError:
            race.append('NA')
            break
    while True:
        try:
            afam.append(model.similarity('equality','african_american'))
            break
        except KeyError:
            afam.append('NA')
            break
...
def iterate_model_stats(list_of_lists, iterations):
    full_stats = []
    earth_model = None
    earth_model = Word2Vec(list_of_lists[0], size = 100, min_count = 0, iter = 200, 
                       sg = 1, hs = 0, negative = 5, window = 10, workers = 4)
    earth_model.wv.init_sims()
    for k in range(0, len(list_of_lists)-1):
        ...
        for i in range(0, iterations):
            ...
            earth = None
        new_earth = None
        new_earth = Word2Vec(list_of_lists[k], size = 100, min_count = 0, iter = 200, 
                     sg = 1, hs = 0, negative = 5, window = 10, workers = 4)
        new_moon = Word2Vec(list_of_lists[k+1], size = 100, min_count = 0, iter = 200, 
                     sg = 1, hs = 0, negative = 5, window = 10, workers = 4)
        new_earth.wv.init_sims()
        new_moon.wv.init_sims()
...
def chrono_train(n_iterations, current_corpus, previous_model, output_model):
    ...
    for k in range(n_iterations):
        ...
        model.train(sentence_samples, total_examples = len(sentence_samples), epochs = model.iter)
